...
export async function getAccessibilityAuthSignals(
    ...
    try {
        // Use DOM-based approach to find aria-labeled auth elements
        const signals = await page.evaluate(() => {
            const authSignals: string[] = [];
            const authKeywords = ['login', 'signin', 'sign in', 'log in', 'password', 'email', 'username', 'register', 'signup', 'sign up'];

            // Check elements with aria-label
            const ariaElements = document.querySelectorAll('[aria-label], [role="button"], [role="link"], [role="textbox"], input, button, a');
            ...
            ariaElements.forEach((el) => {
                ...
                const content = `${ariaLabel} ${text} ${placeholder} ${name}`;

                if (authKeywords.some(kw => content.includes(kw))) {
                    authSignals.push(`${role}: ${ariaLabel || text.slice(0, 50) || name}`);
                }
            });

            return authSignals.slice(0, 20); // Limit results
        });
